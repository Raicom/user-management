<?php

use webvimark\modules\UserManagement\components\GhostHtml;
use webvimark\modules\UserManagement\models\rbacDB\Role;
use webvimark\modules\UserManagement\models\User;
use webvimark\modules\UserManagement\UserManagementModule;
use yii\helpers\Html;
use yii\helpers\ArrayHelper;
use yii\widgets\Pjax;
use webvimark\extensions\GridBulkActions\GridBulkActions;
use webvimark\extensions\GridPageSize\GridPageSize;
use kartik\grid\GridView;
use app\models\States;
use app\models\UserDirection;
use webvimark\modules\UserManagement\models\UserVisitLog;
$token = Yii::$app->session->get('__visitorToken');
if ($token) {
    $log_model = UserVisitLog::find()->where(['token' => $token])->one();
    $log_model->exit_time = time();
    $log_model->duration = gmdate("H:i:s",$log_model->exit_time - $log_model->visit_time);
    $log_model->save(false);
}
/**
 * @var yii\web\View $this
 * @var yii\data\ActiveDataProvider $dataProvider
 * @var webvimark\modules\UserManagement\models\search\UserSearch $searchModel
 */

$this->title = UserManagementModule::t('back', 'Users');
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="user-index">

    <h2 class="lte-hide-title"><?= $this->title ?></h2>

    <div class="panel panel-default">
        <div class="panel-body">

            <div class="row">
                <div class="col-sm-6">
                    <p>
                        <?= GhostHtml::a(
                            '<span class="glyphicon glyphicon-plus-sign"></span> ' . UserManagementModule::t('back', 'Create'),
                            ['/user-management/user/create'],
                            ['class' => 'btn btn-success']
                        ) ?>
                    </p>
                </div>

                <div class="col-sm-6 text-right">
                    <?= GridPageSize::widget(['pjaxId'=>'user-grid-pjax']) ?>
                </div>
            </div>


            <?php Pjax::begin([
                'id'=>'user-grid-pjax',
            ]) ?>

            <?= GridView::widget([
                'id'=>'user-grid',
                'dataProvider' => $dataProvider,
                'pager'=>[
                    'options'=>['class'=>'pagination pagination-sm'],
                    'hideOnSinglePage'=>true,
                    'lastPageLabel'=>'>>',
                    'firstPageLabel'=>'<<',
                ],
                'filterModel' => $searchModel,
                'layout'=>'{items}<div class="row"><div class="col-sm-8">{pager}</div><div class="col-sm-4 text-right">{summary}'.'</div></div>',
                'columns' => [
                    ['class' => 'kartik\grid\SerialColumn', 'options'=>['style'=>'width:10px'],  ],

                    [
                        'class'=>'webvimark\components\StatusColumn',
                        'attribute'=>'superadmin',
                        'visible'=>Yii::$app->user->isSuperadmin,
                    ],

                    [
                        'attribute'=>'username',
                        'value'=>function(User $model){
                            return Html::a($model->username,['view', 'id'=>$model->id],['data-pjax'=>0]);
                        },
                        'format'=>'raw',
                        'vAlign'=>'middle',
                    ],
                    [
                        'attribute'=>'name',
                        'format'=>'raw',
                        'vAlign'=>'middle',
                    ],
                    [
                        'attribute'=>'statesCode',
//                        'header' => 'Office state',
                        'vAlign'=>'middle',
                        'hAlign'=>'center',
                        'width'=>'100px',
                        'value'=>function ($model) {
                            return $model->statesCode;
                        },
                        'filterType'=>GridView::FILTER_SELECT2,
                        'filter'=>ArrayHelper::map(States::find()->orderBy('code')->asArray()->all(), 'code', 'code'),
                        'filterWidgetOptions'=>[
                            'pluginOptions'=>['allowClear'=>true],
                        ],
                        'filterInputOptions'=>['placeholder'=>'State'],
                        'format'=>'raw'
                    ],
                    [
                        'attribute'=>'directionDescr',
//                        'header' => 'Office state',
                        'vAlign'=>'middle',
                        'hAlign'=>'center',
                        'width'=>'100px',
                        'value'=>function ($model) {
                            return $model->directionDescr;
                        },
                        'filterType'=>GridView::FILTER_SELECT2,
                        'filter'=>ArrayHelper::map(UserDirection::find()->orderBy('descr')->asArray()->all(), 'descr', 'descr'),
                        'filterWidgetOptions'=>[
                            'pluginOptions'=>['allowClear'=>true],
                        ],
                        'filterInputOptions'=>['placeholder'=>'Direction'],
                        'format'=>'raw'
                    ],
//                    [
//                        'class' => 'kartik\grid\BooleanColumn',
//                        'width'=>'160px',
//                        'filterType'=>GridView::FILTER_SELECT2,
//                        'trueLabel' => 'Collector',
//                        'falseLabel' => 'Non-collector',
//                        'filter' =>[
//                            true => '1',
//                            false => '0',
//                        ],
//                        'filterWidgetOptions'=>[
//                            'options' => ['placeholder'=>'Show all...'],
//                            'pluginOptions'=>['allowClear'=>true],
//                        ],
//                        'attribute' => 'collector',
//                        'vAlign' => 'middle',
//                        'label' => 'Collector',
//                    ],
//                    [
//                        'attribute'=>'email',
//                        'format'=>'raw',
//                        'vAlign'=>'middle',
//                        'visible'=>User::hasPermission('viewUserEmail'),
//                    ],
//                    [
//                        'class'=>'webvimark\components\StatusColumn',
//                        'attribute'=>'email_confirmed',
//                        'visible'=>Yii::$app->user->isSuperadmin,
//                    ],
//                    [
//                        'attribute'=>'gridRoleSearch',
//                        'filter'=>ArrayHelper::map(Role::getAvailableRoles(Yii::$app->user->isSuperAdmin),'name', 'description'),
//                        'value'=>function(User $model){
//                            return implode(', ', ArrayHelper::map($model->roles, 'name', 'description'));
//                        },
//                        'format'=>'raw',
//                        'vAlign'=>'middle',
//                        'visible'=>User::hasPermission('viewUserRoles'),
//                    ],
                    [
                        'attribute'=>'registration_ip',
                        'value'=>function(User $model){
                            return Html::a($model->registration_ip, "http://ipinfo.io/" . $model->registration_ip, ["target"=>"_blank"]);
                        },
                        'format'=>'raw',
                        'vAlign'=>'middle',
                        'visible'=>Yii::$app->user->isSuperadmin,
//						'visible'=>User::hasPermission('viewRegistrationIp'),
                    ],
//                    [
//                        'value'=>function(User $model){
//                            return GhostHtml::a(
//                                UserManagementModule::t('back', 'Roles and <br> permissions'),
//                                ['/user-management/user-permission/set', 'id'=>$model->id],
//                                ['class'=>'btn btn-sm btn-primary', 'data-pjax'=>0]);
//                        },
//                        'format'=>'raw',
//                        'vAlign'=>'middle',
//                        'visible'=>User::canRoute('/user-management/user-permission/set'),
//                        'options'=>[
//                            'width'=>'10px',
//                        ],
//                    ],
                    [
                        'value'=>function(User $model){
                            return GhostHtml::a(
                                UserManagementModule::t('back', 'Change <br> password'),
                                ['change-password', 'id'=>$model->id],
                                ['class'=>'btn btn-sm btn-default', 'data-pjax'=>0]);
                        },
                        'format'=>'raw',
                        'vAlign'=>'middle',
                        'options'=>[
                            'width'=>'10px',
                        ],
                    ],
                    [
                        'class'=>'webvimark\components\StatusColumn',
                        'attribute'=>'status',

                        'optionsArray'=>[
                            [User::STATUS_ACTIVE, UserManagementModule::t('back', 'Active'), 'success'],
                            [User::STATUS_INACTIVE, UserManagementModule::t('back', 'Inactive'), 'warning'],
                            [User::STATUS_BANNED, UserManagementModule::t('back', 'Banned'), 'danger'],
                        ],
                    ],
//					['class' => 'yii\grid\CheckboxColumn', 'options'=>['style'=>'width:10px'] ],
                    [
                        'class' => 'yii\grid\ActionColumn',
                        'template' => '{view} {update}',
                        'contentOptions'=>['style'=>'width:70px; text-align:center;'],
                    ],
                ],
            ]); ?>

            <?php Pjax::end() ?>

        </div>
    </div>
</div>
